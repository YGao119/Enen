...
class AgentTaskManager(InMemoryTaskManager):
    def __init__(self, agent: StockAgent, notification_sender_auth: PushNotificationSenderAuth):
        super().__init__()
        self.agent = agent
    ...
    def _validate_request(
        ...
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes, StockAgent.SUPPORTED_CONTENT_TYPES
        ):
            logger.warning(
                "Unsupported output mode. Received %s, Support %s",
                task_send_params.acceptedOutputModes,
                StockAgent.SUPPORTED_CONTENT_TYPES,
            )
